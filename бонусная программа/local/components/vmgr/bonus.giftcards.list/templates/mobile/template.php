...
<div class="lk__content">
    <!-- <h1 class="lk-content__h1 common-h1">Подарочные сертификаты</h1> -->
    <p class="lk-content__num-title">Вы можете выбрать подарок на сумму <span><?=$arResult["TOTAL_CREDIT"]?>  ₽</span></p>
    <button class="btn btn-border lk__profile-btn lk__order-btn lk__order-btn--fill reset-btn load-prav">Показать правила оформления</button>
...
<div id="popup-buy-content" style="display: none;">
    ...
    <form class="popup-buy__form">
        ...
        <label class="popup-buy__code">
            ...
            <input class="popup-buy__code-inpt" autocomplete="one-time-code" name="code" type="text">
        ...
        <button style="margin: 0 auto;display: block;" class="btn btn-border lk__profile-btn lk__order-btn lk__order-btn--fill reset-btn popup-buy__btn disable">Продолжить</button>
    </form>
</div>
    ...
    function executePayment(transactionId, productId, nominal, name, handler) {
        ...
        widget.charge({ // options
            ...
            function (options) {
                ...
                formDataPay.append("action", "success_pay");
                formDataPay.append("product_id", productId);
                formDataPay.append("nominal", nominal);
                formDataPay.append("transaction_id", transactionId);
                ...
                .then((data) => {
                    if(data.status) {
                        ...
                        handler.destroy();
                        const successPopup =  new Popup("<div class='alert-wrap-bn'><img src='/local/templates/desktop/images/success.png' /><p>Сертификат в процессе оформления. Как только он будет готов, вы получите его на электронную почту.</p><a class='btn btn-border lk__profile-btn lk__order-btn lk__order-btn--fill reset-btn' href='/personal/bonus/'>Закрыть</a></div>", reloadPageMailBonus);
                        successPopup.init();
                        successPopup.open(null,'');
                    } else {
                        // Случай 3. Оплата прошла, но сертификат не оформился
                        handler.destroy();
                        const successPopup =  new Popup("<div class='alert-wrap-bn'><img src='/local/templates/desktop/images/success.png' /><p>Сертификат в процессе оформления. Как только он будет готов, вы получите его на электронную почту. Оформление сертификата может занимать до 3-х дней.</p><a class='btn btn-border lk__profile-btn lk__order-btn lk__order-btn--fill reset-btn' href='/personal/bonus/'>Закрыть</a></div>", reloadPageMailBonus);
                        successPopup.init();
                        successPopup.open(null,'');
                    }
                });
            ...
            function (reason, options) {
                ...
                const formDataPay = new FormData();
                formDataPay.append("action", "error_pay");
                formDataPay.append("options", options);
                formDataPay.append("reason", reason);
                ...
                .then((res) => res.json())
                .then((data) => {
                    // Случай 2. Оплата не прошла
                    handler.destroy();
                    const successPopup =  new Popup("<div class='alert-wrap-bn'><img src='/local/templates/desktop/images/fatal.png' /><p>Оформление сертификата прервано. Повторите попытку</p><a class='btn btn-border lk__profile-btn lk__order-btn lk__order-btn--fill reset-btn' href='/personal/bonus/giftcards/'>Закрыть</a></div>", reloadPage);
                    successPopup.init();
                    successPopup.open(null,'');
                });
            }
