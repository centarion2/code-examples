...
В вызов компонента news.list добавлены следующие параметры:
 ...
 CLI скрипт удаляет старые записи из хайлоад блока ApiRequests. Старые записи определяются по столбцу UF_DATE_CREATE хайлоада типа "Дата и время". Скрипт использует бесплатный модуль Настройки++ (askaron.settings) для хранения настроек.
 Два варианта скрипта:
 - cleanApiRequestsORM.php - первый скрипт для удаления использует D7 ORM. Он чуть быстрее, чем стандартное битриксовое удаление с помощью методов getList и delete дата класса.
 - cleanApiRequestsSQL.php - второй скрипт для удаления использует прямой SQL запрос DELETE. Также используется дополнителный запрос SELECT для подсчёта и вывода количества удаленных строк.
Оба скрипта имеют два целочисленных параметра:
 - UF_API_REQUEST_STORE_DAYS - сколько дней хранить АПИ-запросы, берётся из настроек,
 - 1000 записей - максимальное число удаляемых строк за один запуск скрипта, Задаётся жестко в коде.
